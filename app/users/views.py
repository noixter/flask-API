...
from app.users.adapters.repository import (
...
class BaseResource(Resource):
    ...
    def dispatch_request(self, *args, **kwargs):
        ...
        if request.user and not any(
            [permission.has_permission(request) for permission in self.permissions]
        ):
            raise PermissionDenied("user is not allowed to perform this action")
...
class User(BaseResource):
    ...
    services = UserServices(
        user_repository=user_repository, role_repository=SQLAlchemyRoleRepository()
    )
    ...
    def put(self, user_id):
        ...
        serialize_user = self.user_serializer.dump(user)
        return {"message": "updated", "user": serialize_user}, 200
...
class Token(Resource):
    ...
    service = JWTLogin(repository=SQLAlchemyUserRepository(), token_handler=JWTHandler())
